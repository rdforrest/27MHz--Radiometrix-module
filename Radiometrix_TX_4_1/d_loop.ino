void loop() {
  ...
  for (int i = 0; i < 6; i++)  //display all channels
  {
    ...
    {
      Serial.print(ppmValues[i]);  // Serial.print(ppmValues[i]);
      Serial.print("\t");          //tab
    }
#endif
    ...
    // Make sure that stick end points on TX are set to 100% and neutrals are set to 50% and that Trainer Function on TX is enabled
